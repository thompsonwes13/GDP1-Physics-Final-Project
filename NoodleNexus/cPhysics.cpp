...
// For BROAD face AABB
bool cPhysics::rayCast(glm::vec3 start, glm::vec3 end, std::string direction,
	std::vector<sCollision_RayTriangleInMesh>& vec_RayTriangle_Collisions,
	std::vector<cPhysics::sTriangle> AABB_Tris,
	bool bIgnoreBackFacingTriangles /*= true*/)
{
	sLine* theRay = new sLine();
	theRay->startXYZ = start;
	theRay->endXYZ = end;
	...
	sCollision_RayTriangleInMesh intersectionInfo;
	intersectionInfo.theRay = *theRay;
	intersectionInfo.rayDirection = direction;

	// Go through each triangle
	for (std::vector<sTriangle>::iterator itTri = AABB_Tris.begin();
								  itTri != AABB_Tris.end(); itTri++)
	{
		loopCount++;
		//std::cout << loopCount << std::endl;
		sTriangle CurTriangle = *itTri;
